...
{-}
records :: forall m a. Monad m
        => Producer ByteString m ()
        -> Producer ([Field], BSL.ByteString) m ()
records prod = do
    let warc = parseWarc $ hoist lift prod
    let run :: forall m' a. (m' ~ Producer ([Field], BSL.ByteString) m)
            => Record m' a -> m' a
        run r = do
            (chunks, rest) <- PP.toListM' $ recContent r
            yield (recHeader r, BSL.fromChunks chunks)
            return rest
    rest <- iterRecords run warc
    return ()

records' :: forall m i o a r. Monad m
        => Consumer i m o
        -> Producer i m r
        -> Producer o m (Producer i m r)
records' consume = _ . f
  where
    f :: Producer i m r -> Producer o m (Producer i m r)
    f prod = do
        mres <- next prod
        case mres of
            Nothing -> return prod
            Just x -> push x >-> consume
...
records' :: forall m a. Monad m
        ...
        -> Producer ([Field], BSL.ByteString) m ()
records' prod = do
    let Warc warc = parseWarc $ hoist lift prod
